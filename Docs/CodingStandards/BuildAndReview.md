...
- Target UE 5.6 and later. Engine compilation CI should cover both the minimum supported release and the latest supported release. The current post-merge CircleCI job builds the Linux plugin with UE 5.6.1; the latest supported release remains a coverage gap. See the [development and CI workflow](../DevelopmentWorkflow.md).
